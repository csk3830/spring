...
public class SecurityConfig {
    /* springSecurity6 => bcEncoder => createDelegatingPasswordEncoder */
        ...
        return http.csrf(csrf-> csrf.disable())
                .authorizeHttpRequests(authorize->
                    authorize.requestMatchers("/user/list").hasAnyRole("ADMIN")
                            .requestMatchers("/","/index","/js/**","/upload/**", "/user/login", "/dist/**", "/image/**", "/board/detail",
                                    "/user/signup", "/board/list", "/comment/list/**").permitAll()
