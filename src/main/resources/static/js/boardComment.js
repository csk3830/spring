...
function spreadCommentList(bnoVal, page=1){
    CommentListGetFromServer(bnoVal, page).then(result=>{
        console.log(result);
...
document.addEventListener('click',(e)=>{
    ...
    if(e.target.id === 'cmtModBtn'){
        ...
        // 비동기 처리
        CommentUpdateToServer(cmtModData).then(result=>{
            if(result >0){
                alert("수정 성공~!!")
            }
...
async function CommentUpdateToServer(cmtModData){
    try{
...
async function CommentListGetFromServer(bno, page){
    try{
